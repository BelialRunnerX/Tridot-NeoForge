...
- **Arrow pickup item during construction** — vanilla's 1.21 `AbstractArrow(EntityType, Level)` constructor calls the virtual `getDefaultPickupItem()` (it stores the pickup stack in a field). Java runs that call before `AbstractTridotArrow`'s own field initialiser sets `arrowItem = ItemStack.EMPTY`, so every Tridot arrow created through `EntityType.create` threw a `NullPointerException` on the server (Valoria's Phantasm Bow fired nothing). `getDefaultPickupItem` now treats a null `arrowItem` like an empty one and returns a plain arrow.
