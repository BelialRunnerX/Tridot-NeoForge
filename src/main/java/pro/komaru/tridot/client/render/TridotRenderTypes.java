...
public class TridotRenderTypes{
    ...
    /**
     * PORT NOTE: BufferBuilders are created per draw in 1.21, so {@code begin} now returns the builder obtained from the
     * tesselator and {@code end} receives it back to upload (the old {@code begin(BufferBuilder, TextureManager)} /
     * {@code end(Tesselator)} pair no longer maps onto the API).
     */
