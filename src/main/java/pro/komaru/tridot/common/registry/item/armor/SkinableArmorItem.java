...
public class SkinableArmorItem extends PercentageArmorItem{
    ...
    // PORT NOTE: getArmorTexture now receives the material Layer and returns a ResourceLocation; the skin API still
    // yields the 1.20.1-style "namespace:path" string, which is parsed here.
    @OnlyIn(Dist.CLIENT)
    @Override
    public ResourceLocation getArmorTexture(ItemStack stack, Entity entity, EquipmentSlot slot, ArmorMaterial.Layer layer, boolean innerModel){
        ...
        // The old Forge "type" argument was null for the base layer and "overlay" for the dyeable overlay; layers no
        // longer expose that, so the base layer is reported as null and the inner (leggings) model as "inner".
