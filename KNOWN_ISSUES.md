...
- **Networking.** `SimpleChannel` → `CustomPacketPayload` + `StreamCodec`; `PacketHandler.addRegistration` still lets other mods register on Tridot's channel. `sendToTracking(chunk)` re-implements Tridot's `TRACKING_CHUNK_AND_NEAR` distributor (players tracking the chunk and within 64 blocks) with the same semantics.
