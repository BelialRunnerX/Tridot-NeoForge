...
- **Предмет подбора стрелы во время конструирования** — конструктор ванильного `AbstractArrow(EntityType, Level)` в 1.21 вызывает виртуальный `getDefaultPickupItem()` (он сохраняет стек подбора в поле). Java выполняет этот вызов до того, как инициализатор поля `AbstractTridotArrow` установит `arrowItem = ItemStack.EMPTY`, поэтому любая стрела Tridot, создаваемая через `EntityType.create`, выбрасывала `NullPointerException` на сервере (Phantasm Bow из Valoria ничего не выпускал). `getDefaultPickupItem` теперь трактует null в `arrowItem` как пустой стек и возвращает обычную стрелу.
